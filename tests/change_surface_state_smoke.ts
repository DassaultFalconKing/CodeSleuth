...
async function main() {
  ...
  await mkdir(path.join(root, "src", "pkg"), { recursive: true }); await mkdir(path.join(root, "tests"), { recursive: true }); await mkdir(path.join(root, "docs"), { recursive: true }); await mkdir(path.join(root, ".github", "workflows"), { recursive: true }); await mkdir(path.join(root, "migrations"), { recursive: true })
  ...
  for (const required of ["src/pkg/service.py", "src/pkg/api.py", "tests/test_service.py", "pyproject.toml", "docs/SESSION.md", "verify.sh"]) {
