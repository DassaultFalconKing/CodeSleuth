...
async function main() {
  ...
  const rendered = JSON.parse(await mermaid.execute({}, context))
  ...
  const legacySource = await mermaid.execute({ responseFormat: "mermaid_source" }, context)
  assert(legacySource === diagram, "legacy source compatibility must be explicit and byte-identical")
