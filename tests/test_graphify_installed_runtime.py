...
def test_disposable_install_carries_provider_tools_and_lifecycle(tmp_path: Path, monkeypatch) -> None:
    ...
    installed = run(sys.executable, str(ROOT / "install.py"), str(tmp_path))
    assert "installed CodeSleuth" in installed.stdout
    smoke = run(sys.executable, str(ROOT / "smoke.py"), str(tmp_path))
    ...
    command = [
        sys.executable, "-m", "codesleuth_project", "--install-graphify-runtime", str(tmp_path)
    ]
    ...
    runtime_identity = json.loads((runtime / "codesleuth-runtime.json").read_text(encoding="utf-8"))
    assert runtime_identity["schemaVersion"] == 1
    assert runtime_identity["provider"] == {"id": "graphify", "version": "0.9.50"}
    assert Path(runtime_identity["pythonExecutable"]).resolve() == Path(sys.executable).resolve()
    (runtime / "marker.txt").write_text("optional", encoding="utf-8")
    removed = subprocess.run(
        [sys.executable, "-m", "codesleuth_project", "--remove-graphify-runtime", str(tmp_path)],
        capture_output=True,
        text=True,
        check=True,
        env=environment,
