...
- `pack/.opencode/tools/repo_context_graph.ts` — OpenCode-native tools:
  - `repo_context_graph_save`: persist/update a bounded projection under
    `.opencode/state/context-graphs/` (below the existing ignored OpenCode
    state boundary; no new tracked or project-authoritative format).
...
- Every source-derived node/edge retains a SourceRef: tracked path, current
  ...
  refused; untracked paths are refused).
