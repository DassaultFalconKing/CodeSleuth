...
**Semantic-refit target:** `bf387965abdf5218b4528f87f0b2d8893fa28418`
**Change class:** post-SIB2 feature population inside the existing
`context-relationship-graph` capability class
**Status:** explicitly admitted as an incubating optional provider for the 0.4.0 release
composition; promotion is established only by exact-refit-head acceptance and EHA
...
- [x] `eha_state_mermaid` preserves the established no-argument source response and
      exposes the versioned JSON envelope through explicit `responseFormat: json`.
...
These are promotion requirements, not durable PASS checkboxes in source documentation.
They must be re-executed and recorded against the exact semantic-refit head: all focused
gates; full Python/Bun/Ruff and lifecycle/packaging coverage; optional-runtime absent and
canonical Ubuntu/Python 3.12 enabled profiles; disposable installed-pack parity; and
explicit skipped-platform/residual-limit reporting. A prior feature-branch run never
transfers acceptance to a refit or release SHA.
