...
Load `exact-target-identity` and `eha-candidate-selection`. For normal future-SIB selection, resolve the active `dev/release-X.Y.Z` literal head SHA and verify the checkout's literal `git rev-parse HEAD` equals that selected SHA.
