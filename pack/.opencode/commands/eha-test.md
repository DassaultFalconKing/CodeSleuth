...
For a trusted GitHub EHA bridge request, run only `python scripts/eha_candidate_status.py` for Step 1 and use its bounded JSON as `candidate_identity`. The bridge has already fetched and frozen `refs/remotes/origin/dev/release-X.Y.Z`, created a fresh durable review checkpoint, and appended the exact-target `campaign_started` event before provider/model execution. Do not rediscover refs, inspect local convenience branches, enumerate the host persistence root, or create a second campaign.
...
Immediately after the bounded candidate check, load `review_state` and `eha_state`. If a fresh incomplete campaign already exists for the same literal target SHA and branch, consume that campaign and bind `provenance_state` once for the current producer session. Only when no matching campaign exists outside trusted-bridge mode may you start a new review checkpoint and call `eha_state_start_campaign`.
