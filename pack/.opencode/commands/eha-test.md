...
Read `docs/EHA-OPERATING-PLAYBOOK.md`, `.opencode/PROVENANCE-WATERMARK.md`, and `docs/PLAYBOOK-SKILL-COMMAND-TOOL-CONTRACT.md`, then resolve `pack/.opencode/playbooks/eha-sib-acceptance/playbook.json`.
...
Before testing, start a fresh `review_state` checkpoint, call `provenance_state_bind` once with the stable opaque actor for this producer session, then call `eha_state_start_campaign`. Run the SIB0, SIB1, and SIB2 profiles against the SAME immutable SHA. Do not modify application/source files during this command. A failure is a valid EHA result, not permission to repair the target in place.
