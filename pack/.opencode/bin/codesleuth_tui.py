...
NAV_SURFACES = {
    ...
    "evidence": (
        ...
        "Evidence, findings, coverage, durable checkpoints, and `.codesleuth/reports/` remain OpenCode-written. Launch OpenCode to inspect, resume, or persist them.",
    ),
    "tools": (
    ...
    "settings": (
        ...
        "Configure repository profiles, OpenCode agent/model profile, explicit evidence permissions, runtime policy, and the optional pinned dependency.",
    ),
}
...
HELP_SECTIONS = [
    (
        "What CodeSleuth is",
        "CodeSleuth is a control panel over OpenCode, not a second supervisor. "
        "The TUI selects profiles, skills, commands, permissions, tools, and models. "
        "OpenCode's primary build agent remains the controller, with a native prompt chosen by the selected model.",
    ),
    (
        "Quick start",
    ...
    (
        ...
        "OpenCode's primary build agent loads it from /repo-review or /repo-review-resume and follows its inventory, architecture, "
        "evidence-ledger, checkpoint, context-discipline, and completion contracts. "
        "Custom CodeSleuth agents are Task subagents, not a replacement controller.",
    ),
    (
        "Playbooks",
