...
export const mermaid = tool({
  description: "Render a bounded Mermaid flowchart of EHA targets, SIB verdicts, and repair lineage. Returns a versioned JSON envelope by default; request mermaid_source only for explicit legacy source compatibility. This is a derived presentation of the evidence ledger, never acceptance authority itself.",
  args: {
    reviewId: tool.schema.string().optional(),
  ...
  async execute(args, context) {
    ...
    if (args.responseFormat === "mermaid_source") return mermaidSource
